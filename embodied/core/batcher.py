...
class Batcher:
  ...
  def __init__(
      ...
      prefetch_source=4, prefetch_batch=2, batch_shape=None):
    # prod(batch_shape) has to equal the length of the sources
    if batch_shape:
      assert np.prod(batch_shape) == len(sources), (batch_shape, len(sources))
  ...
  def __next__(self):
    ...
    # NOTE: Added batch shape api here
    if self._batch_shape is not None:
      # (prod(*B), ...) => (*B, ...)
      # [print(f"{k}: {v.shape}", end=", ") for k, v in batch.items()]
      batch = {k: np.reshape(v, (*self._batch_shape, *v.shape[1:])) for k, v in batch.items()}
  ...
  def _batcher(self, sources, output):
    try:
      ...
      print(f"batcher: elems[0] is {type(elems[0])}, len(elems): {len(elems)}, dims in every elems:")
      for k in elems[0].keys():
        print(f"Key {k}: ", end="")
        for i in range(len(elems)):
          print(f"{elems[i][k].shape}", end=", ")
        print()
      e.stacktrace = ''.join(traceback.format_exception(*sys.exc_info()))
      output.put(e)
      raise
