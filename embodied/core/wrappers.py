...
class TransitionDelta(base.Wrapper):
  ...
  def step(self, action):
    ...
    # return {k: convert(v) for k, v in obs.items()}
    return obs

# TODO: Do this as a wrapper
class EnvNormalizer:
  def __init__(self, env, keys=r'.*') -> None:
    super().__init__(env)
...
class GymWrapperFinalLayer(gym.Env):
  def __init__(self, env: base.Env, obs_key="state"):
  ...
  def observation_space(self):
    return gym.spaces.Box(self.env.obs_space[self.obs_key].low, self.env.obs_space[self.obs_key].high)
  ...
  def step(self, action):
    ...
    info = {}
    return o, rew, terminal, truncated, info
